  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">{{ scope.row.articleUpdateTime | formatDateTime}}</template>
      </el-table-column>
    ...
    methods: {
      ...
      deleteMany(){
        ...
        for (var i = 0; i < selItems.length; i++) {
          this.dustbinData.push(selItems[i].articleId)
        }
      ...
      loadBlogs(page, count){
        ...
        var _this = this;
        var url = '';   //state一定要对应数据表Article的article_status 字段  1为显示  0为草稿 
       
		
			
			 	
        url = "/Article/allArticle" + "?pageNum=" + this.currentPage + "&pageSize=" + this.pageSize + "&keywords=" + this.keywords+"&state="+_this.state;
        console.log(url)
        getRequest(url).then(resp=> {
          ...
          _this.$message({type: 'error', message: '数据加载失败!'});
			    ...
			    var  url = "/Article/allArticle" + "?pageNum=" + this.currentPage + "&pageSize=" + this.pageSize + "&keywords=" + this.keywords+"&state="+_this.state;
			     console.log(url)
			     getRequest(url).then(resp=> {
			       _this.loading = false;
			       if (resp.data.total != 0) {
      ...
      handleEdit(index, row) {
        this.$router.push({path: '/editBlog', query: {from: this.activeName,id:row.articleId}});
      },
      handleDelete(index, row) {
        this.dustbinData.push(row.articleId);
        this.deleteToDustBin(row.state);
      },
      deleteToDustBin(state){
        var _this = this;
        this.$confirm(state != 2 ? '永久删除该文件, 是否继续?' : '永久删除该文件, 是否继续?', '提示', {
